...
\note{Call on students and ask them what the type of each expression is. For the third and fourth bullets (0 and 4.0), get different opinions and ask students to explain why they think it's double/int. Then, of course, explain what the right answer is and why.}
\begin{itemize}
\item<3->9 + 1
\item<4->8.3 - 10.2
\item<5->0
...
\frametitle{Example problem: write a function}
...
The area of a triangle with base $b$ and height $h$ is given by $\left(\frac{b\times{h}}{2}\right)$. Write a function called areaTriangle that calculates the area of a triangle given $b$ and $h$.
\pause
(do this problem in a separate text editor before proceeding)
\pause
\begin{lstlisting}
private double areaTriangle(double b, double h) {
    return b * h / 2;
}
\end{lstlisting}
\end{frame}
